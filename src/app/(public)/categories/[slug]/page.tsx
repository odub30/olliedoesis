...
import { ArrowLeft, Folder, FileText, Briefcase } from "lucide-react";
import type { Metadata } from "next";
import { getContentByCategory } from "@/lib/queries/content";
...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  const content = await getContentByCategory(slug);
  ...
  const totalItems = content.blogs.length + content.projects.length;
  const description = content.category.description ||
    `Explore ${totalItems} pieces of content in the "${content.category.name}" category: ${content.blogs.length} blog posts and ${content.projects.length} projects.`;
  const canonicalUrl = getCanonicalUrl(`categories/${slug}`);

  return {
    title: `${content.category.name} - All Content | Ollie Doesis`,
    description,
    keywords: content.category.name,
    openGraph: {
      title: `${content.category.name} - All Content`,
      description,
      type: "website",
      url: canonicalUrl,
    },
          ...
          <div className="flex items-start gap-4 mb-6">
            <div className="flex-shrink-0 w-16 h-16 bg-white/10 backdrop-blur-sm rounded-2xl flex items-center justify-center border border-white/20">
              {content.category.icon ? (
                <span className="text-3xl">{content.category.icon}</span>
              ) : (
          ...
          {totalItems === 0 && (
            <div className="bg-white rounded-2xl shadow-2xl p-12 text-center">
              ...
              <p className="text-muted-foreground mb-6">
                There are no published items in the "{content.category.name}" category yet.
              </p>
              <Link
                href="/blogs"
